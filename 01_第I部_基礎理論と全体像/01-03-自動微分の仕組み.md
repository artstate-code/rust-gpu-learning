...
**記号微分**（Symbolic Differentiation）は、数式を代数的に変形して導関数を求める手法です。
...
print(f"f2' の項数: {len(sp.Add.make_args(f2_prime))}")  # 10項
...
print(f"g の項数: {len(sp.Add.make_args(g_expanded))}")  # 数千項
...
print(f"g' の項数: {len(sp.Add.make_args(g_prime))}")  # さらに増加
```
...
**結論**: 記号微分は式が爆発的に複雑になるため、大規模な数値計算（機械学習など）には不向きです。自動微分は計算グラフを直接実行するため、式の複雑さに依らず効率的です。
