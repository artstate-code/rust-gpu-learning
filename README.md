...
テンソル演算、ニューラルネットワークの基本レイヤー、損失関数、データローディング、学習ループを実装し、実用的な ML エンジンを構築します。
...
#### [第 10 章：ニューラルネットワーク基本レイヤーの実装](./04_第IV部_機械学習エンジンの構築/04-10-ニューラルネットワーク基本レイヤーの実装.md)

- 10.1 活性化関数の実装と GPU 最適化（ReLU, Sigmoid, Tanh, GELU, Swish）
- 10.2 正規化層の実装（BatchNorm, LayerNorm, GroupNorm）
- 10.3 プーリング層の実装（MaxPool, AvgPool, AdaptivePool）
- 10.4 ドロップアウトと正則化
- 10.5 まとめとパフォーマンス比較

#### [第 11 章：損失関数とメトリクスの実装](./04_第IV部_機械学習エンジンの構築/04-11-損失関数とメトリクスの実装.md)

- 11.1 回帰タスクの損失関数（MSE, MAE, Huber Loss）
- 11.2 分類タスクの損失関数（BCE, Cross Entropy, Focal Loss）
- 11.3 カスタム損失関数の設計パターン（Multi-task, 正則化, Contrastive）
- 11.4 評価メトリクスの実装（Accuracy, Precision, Recall, F1, R²）
- 11.5 数値安定性の考慮

#### [第 12 章：データローディングとバッチ処理](./04_第IV部_機械学習エンジンの構築/04-12-データローディングとバッチ処理.md)

- 12.1 Dataset と DataLoader の設計
- 12.2 データの前処理とデータ拡張（Transform, Augmentation）
- 12.3 効率的な GPU メモリ転送（Pinned Memory, Stream）
- 12.4 メモリ効率的なバッチ処理（Gradient Accumulation）
- 12.5 並列データローディングとプリフェッチ

#### [第 13 章：学習ループと最適化手法](./04_第IV部_機械学習エンジンの構築/04-13-学習ループと最適化手法.md)

- 13.1 forward → loss → backward → optimizer の流れ
- 13.2 SGD / Adam / AdamW / RMSProp の実装
- 13.3 勾配クリッピング・学習率スケジューラ
- 13.4 mixed precision（FP16/BF16）・量子化・sparsity
- 13.5 勾配チェックポイント（Gradient Checkpointing）
- 13.6 Rust での再現: burn / tch-rs 内部構造解析

#### [第 14 章：デバッグとプロファイリング](./04_第IV部_機械学習エンジンの構築/04-14-デバッグとプロファイリング.md)

- 14.1 数値計算の検証（Python出力との比較）
- 14.2 GPU プロファイリング（NVIDIA Nsight, rocm-profiler）
- 14.3 CPU プロファイリング（cargo-flamegraph, perf）
- 14.4 メモリリーク・競合検出（valgrind, miri, sanitizers）
- 14.5 テスト自動化と CI/CD
- 14.6 エラーハンドリングのベストプラクティス
...
#### [第 15 章：モデル推論と ONNX 互換](./05_第V部_応用と高度化/05-15-モデル推論とONNX互換.md)

- 15.1 推論と学習の違い
- 15.2 ONNX フォーマットの理解
- 15.3 Rust での ONNX 推論（tract, onnxruntime-rs）
- 15.4 推論最適化テクニック（Graph Fusion, 量子化）
- 15.5 動的バッチング
- 15.6 推論サーバの構築

#### [第 16 章：コンパイラ最適化と DSL 設計](./05_第V部_応用と高度化/05-16-コンパイラ最適化とDSL設計.md)

- 16.1 機械学習コンパイラの概要
- 16.2 JIT コンパイルと実行時最適化
- 16.3 Rust マクロによる DSL 設計
- 16.4 LLVM / MLIR による最適化パイプライン
- 16.5 Triton 風のカーネル記述言語
- 16.6 テンプレートメタプログラミングと型レベル計算

#### [第 17 章：分散・クラスタ対応](./05_第V部_応用と高度化/05-17-分散・クラスタ対応.md)

- 17.1 なぜ分散学習が必要か？
- 17.2 データ並列（Data Parallelism, ZeRO）
- 17.3 Pipeline Parallelism（GPipe）
- 17.4 Tensor Parallelism（Megatron-LM）
- 17.5 NCCL と集団通信（AllReduce, AllGather, Reduce-Scatter）
- 17.6 RPC・gRPC によるノード通信
- 17.7 フォールトトレランスとチェックポイント

#### [第 18 章：セキュリティと信頼性](./05_第V部_応用と高度化/05-18-セキュリティと信頼性.md)

- 18.1 メモリ安全と GPU クラッシュハンドリング
- 18.2 FFI 境界の監査・unsafe 最小化
- 18.3 ライブラリ設計における抽象化戦略
- 18.4 ベンチマーク・テスト自動化・再現性確保
- 18.5 Differential Testing と Fuzzing
...
#### [第 19 章：CNN・RNN・Transformer を実装する](./06_第VI部_ケーススタディと実践/06-19-CNN・RNN・Transformerを実装する.md)

- 19.1 Convolution 層とバックプロパゲーション
- 19.2 Im2Col / Winograd / FFT による畳み込み最適化
- 19.3 LSTM/GRU のメモリアクセス最適化
- 19.4 Transformer の注意機構を Rust で構築
- 19.5 Flash Attention / Multi-Query Attention の実装
- 19.6 推論速度・メモリ比較

#### [第 20 章：エンドツーエンド最適化](./06_第VI部_ケーススタディと実践/06-20-エンドツーエンド最適化.md)

- 20.1 プロファイル解析とボトルネック検出
- 20.2 カーネル最適化・パイプライン分割
- 20.3 演算子融合とグラフ最適化
- 20.4 モデルデプロイ・推論サーバ構築
- 20.5 WebGPU 経由でのブラウザ推論
- 20.6 組み込み・エッジデバイスへの展開

#### [第 21 章：実践プロジェクト](./06_第VI部_ケーススタディと実践/06-21-実践プロジェクト.md)

- 21.1 画像分類モデルの学習と推論
- 21.2 自然言語処理タスクの実装
- 21.3 強化学習エージェントの構築
- 21.4 生成モデル（VAE / GAN）の実装
- 21.5 パフォーマンス比較とベンチマーク
...
#### [第 22 章：Rust での ML エコシステムの進化](./07_第VII部_展望と設計指針/07-22-RustでのMLエコシステムの進化.md)

- 22.1 Burn / Candle / Linfa / dfdx の方向性
- 22.2 PyTorch/TensorFlow との API 互換戦略
- 22.3 Rust が担う高信頼 ML インフラの未来
- 22.4 コミュニティとオープンソース活動
- 22.5 今後の学習ロードマップ
...
5. **第 IV 部**で実際の機械学習エンジンを構築する（第 9-14 章）
6. **第 V 部**で応用技術（分散学習、コンパイラ最適化）を学ぶ（第 15-18 章）
7. **第 VI 部**でケーススタディと実践プロジェクトに取り組む（第 19-21 章）
8. **第 VII 部**でエコシステムの全体像と今後の展望を把握する（第 22 章）

### 学習の進め方

**初学者向け**：第 0 部から順番に読み進めることをお勧めします。特に、第 0 章から第 3 章までは本書全体の基礎となるため、丁寧に読み進めてください。

**中級者向け**：既に CUDA や GPU プログラミングの経験がある方は、第 II 部から始めて、Rust 特有の実装パターンを重点的に学ぶことができます。第 IV 部（第 9-14 章）では、ニューラルネットワークの基本レイヤー、損失関数、データローディングなど、実装に必要な要素を体系的に学べます。

**上級者向け**：特定のトピック（分散学習、コンパイラ最適化など）について深く学びたい方は、第 V 部（第 15-18 章）から直接アクセスできますが、第 III 部・第 IV 部の理解を前提としています。
