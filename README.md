...
実装に役立つ参考資料と早見表を収録しています。

- [付録A：数値最適化（SGD, Adam, RMSProp の導出）](./付録/A-数値最適化（SGD,%20Adam,%20RMSProp%20の導出）.md)
- [付録B：CUDAメモリAPI早見表](./付録/B-CUDAメモリAPI早見表.md)
- [付録C：Rust unsafe 設計パターン](./付録/C-Rust-unsafe-設計パターン.md)
- [付録D：参考実装リポジトリ一覧](./付録/D-参考実装リポジトリ一覧.md)
